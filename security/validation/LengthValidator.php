...
use SaQle\Security\Validation\Types\ValueTypeResolver;
...
class LengthValidator extends ValidatorDecorator {
	 ...
	 /*public function validate($input, $config, $code, $message = null){
		 ...
		 $type = ValueTypeResolver::resolve($feedback['value']);

         $handler = app()->rules->get('length', $type);

         $result = $handler->validate($feedback['value'], $config);

         $feedback['is_valid'] = $result['is_valid'];
