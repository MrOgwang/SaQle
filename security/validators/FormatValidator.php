...
class FormatValidator extends ValidatorDecorator{
	 use ScalarValidator;
	 ...
	 public function validate($input, $config, $code, $message = null){
		 $feedback = $this->execute_next_validator($input, $config, $code, $message);
	 	 $this->validate_scalar($feedback, [$this, 'is_format_valid']);
		 return $feedback;
	 }
	 public function is_format_valid($input, $config){
		 return [
		     'is_valid' => true, 
			 'error' => $this->get_readable_field_name($config['field_name']). ' is not of a valid format.'
		 ];
	 }
}
