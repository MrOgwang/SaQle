...
trait DateUtils{
	 ...
	 private static function get_d_value($name, $current_date = null){
	 	...
	 	$date_format_parts = explode(self::get_separator(), config('app.system_date_format'));
     ...
     public static function time_diff($time_one, $time_two){
		 ...
		 $first->setTimezone(new \DateTimeZone(config('app.timezone')));
		 $second = new \DateTime("@".$time_two);
		 $second->setTimezone(new \DateTimeZone(config('app.timezone')));
