...
abstract class TableSchema implements ITableSchema{
	  ...
	  * Make temporary
	  * */
	 public function set_temporary(bool $temp){
	 	 $this->temporary = $temp;
	 }
	  ...
	  * */
	 public function get_actual_db_column_names(){
	 	return $this->meta['actual_db_columns'];
	 }
	 ...
	 private function initialize(){
         ...
         $this->meta['file_field_settings'] = [];
         $this->meta['defined_field_names'] = [];
         $this->meta['db_columns'] = [];
         $this->meta['actual_db_columns'] = [];
		 ...
		 foreach($this->meta['fields'] as $_k => $mf){
			 ...
			 $mf->set_model_class_pk($this->meta['pk_name'] ?? ''); //a table may not have a primary key
			 //initialize state
			 $mf->initialize();
			 ...
			 if($mf instanceof FileField){
		 	 	 ...
		 	 	 $kwargs = $mf->get_kwargs();
		 	 	 $this->meta['file_field_settings'][$_k] = [
		 	 	 	 'path' => $this->get_file_related_fields($kwargs['path'] ?? ""), 
		 	 	 	 'rename' => $this->get_file_related_fields($kwargs['rename'] ?? ($kwargs['rename_callback'] ?? "")), 
		 	 	 	 'dpath' => $this->get_file_related_fields($kwargs['dpath'] ?? "")
		 	 	 ];
			 ...
			 if(!$isnav_col){
			 	 $this->meta['actual_db_columns'][$_k] = $mf->get_db_column_name();
			 }
		 }
	 }

     public function get_file_field_settings(){
     	 return $this->meta['file_field_settings'];
     }

     private function get_file_related_fields(string $file_config){
     	 $matches = [];
	     //Use regex to find everything inside brackets
	     if(preg_match('/\(([^)]*)\)/', $file_config, $matches)) {
	         //Check if there's content inside the brackets
	         if(trim($matches[1]) === ''){
	             return []; // Return an empty array if the brackets are empty
	         }
	        
	         //Split the matched string by commas and trim spaces
	         return array_map('trim', explode(',', $matches[1]));
	     }

	     //Return an empty array if no brackets were found
	     return [];
     }
