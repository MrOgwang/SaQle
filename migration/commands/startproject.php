...
class StartProject{
     ...
     private function get_authservice_file_contents($name){
           ...
           $content .= "\n\t\tif(!$"."clearpass){";
           $content .= "\n\t\t\t$"."clearpass = $"."this->request->data->get_or_fail('enteruserpassword');";
           $content .= "\n\t\t}";
           $content .= "\n";
           $content .= "\n\t\t$"."username = $"."this->request->data->get('email_address', '');";
           $content .= "\n\t\tif(!$"."username){";
           $content .= "\n\t\t\t$"."username = $"."this->request->data->get_or_fail('enterusername');";
           $content .= "\n\t\t}";
     ...
     private function get_userschema_file_contents($name){
           ...
           $content .= "\nuse SaQle\Orm\Entities\Field\Types\{TextField, IntegerField, FileField, OneToOne, OneToMany, ManyToMany, DateField, BooleanField};";
