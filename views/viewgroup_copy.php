...
class ViewGroup{
	 ...
	 */
	 public function __construct(string $title, string $dao, DbContext $context, ?stdClass $sdata = null, int $gindex = -1){
	 	 $dmodel                = new Model(new $dao());
		 $this->title           = $title;
		 $this->dao             = $dao;
		 $this->table           = $context->get_dao_table_name($dao);
		 $this->primary_field   = $dmodel->get_primary_key_name();
		 $this->rows            = $this->extract_view_rows_from_dao($dao, $context, $sdata, $sdata, $gindex);
		 $dmodel                = null;
	 }
	 ...
	 private function create_child_group($title, $dao, $context, $sdata, $gindex, ?array $settings = null){
     ...
     private function extract_view_rows_from_dao(string $dao, $context, $sdata, $del_data, $gindex){
		 ...
		 foreach($properties as $p){
	         ...
	         $row_label        = ucwords(str_replace("_", " ", $fn_sproperty));
		 	 ...
		 	 if(!$fn_key_instance->get_multiple()){
		 	 	$rows = array_merge($rows, $this->extract_view_rows_from_dao($fdao, $context, $sdata ? $sdata->$fn_sproperty : $sdata, $sdata, $gindex));
		 	 ...
		 	 $fdao_instance   = new $fdao();
		 	 $name_property   = $fdao_instance->get_name_property();
